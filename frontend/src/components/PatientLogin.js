...
import {
  ...
  InputAdornment, IconButton, Link, CircularProgress, Alert, Avatar
} from '@mui/material';
import { Visibility, VisibilityOff, Email, Phone, Lock, Favorite, Person } from '@mui/icons-material';
...
export default function PatientLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const result = await makeApiRequest(API_ENDPOINTS.PATIENT_LOGIN, loginData, 'POST');
      ...
      if (result.success) {
        ...
        if (result.data.token || result.data.access_token) {
          localStorage.setItem('patient_token', result.data.token || result.data.access_token);
        }
        setSuccess(true);
      } else {
  ...
  if (success) {
    return (
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e0f2fe 0%, #f0fdf4 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <Card sx={{ p: 4, minWidth: 320, maxWidth: 400, boxShadow: 3, borderRadius: 3 }}>
          <CardContent>
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e0f2fe 0%, #f0fdf4 100%)' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2, mb: 4 }}>
        <Card sx={{ minWidth: 320, maxWidth: 400, width: '100%', boxShadow: 3, borderRadius: 3 }}>
          <CardContent>
            <form autoComplete="on" onSubmit={handleLogin}>
              ...
              <Button
                ...
                fullWidth
                variant="contained"
                sx={{ mt: 1, mb: 2, height: 48, fontWeight: 'bold', fontSize: 16, background: '#3b82f6', '&:hover': { background: '#2563eb' } }}
                aria-label="Login"
